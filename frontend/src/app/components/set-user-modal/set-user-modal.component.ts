...
export class SetUserModalComponent {
  ...
  open(content){
    this.modalService.open(content, { centered: true }).result.then((result) => {
      ...
      this.loginService.postUserInfo(this.model.firstName, this.model.lastName, self, this.openCallback);
    }, (reason) => {
  ...
  // Called when the loginService has tried to create
  // the account for the user
  openCallback(response){
    alert('Error setting user info: ' + response);
  }
